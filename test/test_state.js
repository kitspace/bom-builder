...
describe('bom_edit lines actions', () => {
  describe('lines', () => {
    it('adds a line', () => {
      ...
      const lines2 = linesReducer(data.merge({lines: lines1}), {
        type: 'addLine',
        value: emptyLine
      }).get('lines')
      assert(lines1.size === 0)
    ...
    it('removes a line', () => {
      ...
      const lines1 = data.get('lines')
      assert(lines1.size === 0)
      let lines2 = linesReducer(data.set('lines', lines1), {
        type: 'addLine',
        value: emptyLine
      }).get('lines')
      lines2 = linesReducer(data.set('lines', lines2), {
        type: 'addLine',
        value: emptyLine
      }).get('lines')
      lines2 = linesReducer(data.set('lines', lines2), {
      ...
      const lines3 = linesReducer(data.set('lines', lines2), {
        type: 'removeLine',
        value: 0
      }).get('lines')
      assert(lines1.size === 0)
  ...
  describe('TSV', () => {
    it('lets you set from TSV', () => {
      ...
      const lines = linesReducer(initialState.data, {
        type: 'setFromTsv',
        value: tsv
      }).get('lines')
  ...
  describe('sorting', () => {
    ...
    beforeEach('set order', () => {
      data2 = linesReducer(data1, {
        type: 'addLine',
        value: emptyLine.set('reference', 'C')
      })
      data2 = linesReducer(data2, {
        type: 'addLine',
        value: emptyLine.set('reference', 'B')
      })
      data2 = linesReducer(data2, {
        type: 'addLine',
        value: emptyLine.set('reference', 'A')
      })
    ...
    it('sorts by reference', () => {
      const lines3 = linesReducer(data2, {
        type: 'sortBy',
        value: 'reference'
      }).get('lines')
